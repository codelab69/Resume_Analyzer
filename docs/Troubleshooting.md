...
| `check_frontend.py` exits with "Playwright is not installed" | It is a dev tool that pulls three browser binaries, so it is deliberately out of `requirements.txt` | `pip install playwright`, then `playwright install chromium firefox webkit`. `scripts/check_frontend.py` is the only way §6 of [[Complete Testing Plan]] gets run; without it those rows go back to being reasoned about |
| `check_frontend.py` cannot reach the frontend | Vite binds IPv6 `[::1]` by default, and Firefox and WebKit resolve `localhost` to IPv4 first | Start it as `npm run dev -- --host 127.0.0.1`. Chromium papers over this; the other two do not |
